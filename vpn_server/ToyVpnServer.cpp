...
//https://zhuanlan.zhihu.com/p/111610535
//https://justinyan.me/post/4009
#ifdef __linux__
...
int main(int argc, char **argv) {
    ...
    while ((tunnel = get_tunnel(argv[2], argv[3])) != -1) {
        ...
        for (int i = 0; i < 3; ++i) {
            send(tunnel, parameters, sizeof(parameters), SO_NOSIGPIPE);
        }
        ...
        while (true) {
            ...
            if (length > 0) {
                ...
                send(tunnel, packet, length, SO_NOSIGPIPE);
            ...
            if (idle) {
                ...
                if (timer < -16000) {
                    ...
                    packet[0] = 0;
                    for (int i = 0; i < 3; ++i) {
                        send(tunnel, packet, 1, SO_NOSIGPIPE);
                    }
                    // Switch to sending.
                    timer = 1;
                }
